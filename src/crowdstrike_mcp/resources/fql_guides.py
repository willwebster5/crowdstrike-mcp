...
- `get <path>` — queue a file retrieval (pull to the Falcon cloud; then use rtr_list_files + rtr_get_extracted_file_contents to download)
...
- `memdump` — process memory dump (writes to session working dir, pull via `get`)
...
4. If a file was pulled via `get`: `rtr_list_files(session_id)` → `rtr_get_extracted_file_contents(session_id, sha256)`.
